...
@section('title','Halaman Ketersediaan Admin')
@section('nav')
    ...
    <div class="container-fluid">		
		...
		<div class="row heading-bg">
			<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
			  <h5 class="txt-dark">Data Ketersediaan</h5>
			</div>
			<!-- Breadcrumb -->
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="panel panel-default card-view">
					<td>
						<a class="btn  btn-primary btn-rounded" href="{{ route ('ketersediaanAdmin.create') }}">Tambah Data</a>
					</td>
							...
							<div class="table-wrap">
								<div class="table-responsive">
									<table id="datable_1" class="table table-hover display" >
										...
										<tbody>
											@foreach ($index as $ketersediaan)
											<tr>
                                                <td>{{ $ketersediaan->ketersediaan }}</td>
												<td>{{ $ketersediaan->deskripsi }}</td>
												<td>{{ $ketersediaan->created_at }}</td>
												<td>{{ $ketersediaan->updated_at }}</td>
												<td>
													<form action="{{ route('ketersediaanAdmin.destroy', $ketersediaan->ketersediaan) }}" method="POST">
													    <a href="{{ route('ketersediaanAdmin.show',$ketersediaan->ketersediaan) }}" class="btn  btn-success btn-rounded">Detail</a>
													    <a href="{{ route('ketersediaanAdmin.edit',$ketersediaan->ketersediaan) }}" class="btn btn-warning btn-rounded">Edit</a>
													    @csrf
                    								    @method('DELETE')
